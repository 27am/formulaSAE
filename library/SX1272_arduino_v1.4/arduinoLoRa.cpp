...
#include "SPI.h"
#include "../arduino-api-v1_4/arduinoMultiprotocol.h"
#include "../arduino-api-v1_4/arduinoUtils.h"
...
uint8_t SX1272::ON()
{
  ...
  Utils.setMUXSocket0(); //COMMENTATO!!!
...
int8_t SX1272::setPower(char p)
{
  ...
  { // FSK Std y mode to write in registers
	  writeRegister(REG_OP_MODE, FSK_STANDBY_MODE);
  }
...
*/
  ...
  if( value == _power )   // #### ATTENZIONE #### fallisce questa uguaglianza...perchè?
  {
	  state = 0;
	  #if (SX1272_debug_mode > 1)
  ...
  else
  {
	  ...
	  Serial.print(F("## value read from register: "));
	  Serial.print(value, HEX);
	  Serial.print(F(" while value that had to be inserted was: "));
	  Serial.print(_power, HEX);
	  Serial.println();
	  Serial.println();
  }
...
int8_t SX1272::setPacketLength(uint8_t l)
{
	...
	if( packet_sent.length == value ) //ogni tanto fallisce l'ugluaglianza ed ogni tanto no....
	{
		state = 0;
		#if (SX1272_debug_mode > 1)
	...
	{
		state = 1;
		Serial.println(F(" errore nel settare la lunghezza del pacchetto"));
...
uint8_t SX1272::setPacket(uint8_t dest, uint8_t *payload)
{
	...
	Serial.println();
	Serial.print(F("END OF 'setPacket' METHOD!"));
...
uint8_t SX1272::sendPacketTimeout(uint8_t dest, char *payload)
{
	...
	if (state == 0)								// and writing it in FIFO.
	{
		...
		Serial.println("** 'setPacket' went OK, now I am doing 'sendWithTimeout' !!");

		state = sendWithTimeout();	// Sending the packet

		
		Serial.println("*///*!!!sei riuscito a spedire sto pacchetto!!");

	} else {

		Serial.println("mannaggia, 'setPacket' failed!");
	}

	Serial.println(F("Ending of 'sendPacketTimeout'"));
